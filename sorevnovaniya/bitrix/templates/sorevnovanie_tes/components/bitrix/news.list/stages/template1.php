...
foreach($arResult["ITEMS"] as $key => $arItem) {
	$sum = 
        (
        floatval($arItem["DISPLAY_PROPERTIES"]["STAGE_1"]["VALUE"])
	...
	<?foreach($arResult["ITEMS"] as $arItem):?>
		<tr>
			...
			<td><?=str_replace('.', ',', $arItem["DISPLAY_PROPERTIES"]["STAGE_6"]["VALUE"])?></td>
		  	<?}?>
            <!--<? if ($arItem["PROPERTIES"]["STAGE_7"]){ ?>	
			<td><?=str_replace('.', ',', $arItem["DISPLAY_PROPERTIES"]["STAGE_7"]["VALUE"])?></td>
		  	<?}?>-->
